...
if(isset($_GET['type']) && $_GET['type']!=''){
	...
	if($type=='status'){
		...
		$update_status_sql="update adminusers set status='$status' where id='$id'";
		mysqli_query($con,$update_status_sql);
	}
	
	if($type=='delete'){
		$id=get_safe_value($con,$_GET['id']);
		$delete_sql="delete from adminusers where id='$id'";
		mysqli_query($con,$delete_sql);
	}
}

$sql="select * from adminusers where role=1 order by id desc";
